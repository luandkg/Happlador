...
public class HapploViewer {
	...
	private Transferidor mTransferidorC;
	private Alinhamento mAlinhamentoC;

	public HapploViewer(int eX,int eY,int eLargura, int eAltura, HapploDocumento eHapploDocumento) {
		mSelecionado = false;
		...
		setFonte(1, Color.white);
		
		mMovimentador= new Movimentador(this);
		mTransferidorC = new Transferidor(this);
		mAlinhamentoC = new Alinhamento(this);
		
		
		if (HapploDocumentoC.getCapacidade()==0) {
			HapploDocumentoC.setCapacidade(getCapacidade());
		}
	...
	public Movimentador getMovimentador() {
		return mMovimentador;
	}
	
	public Transferidor getTransferidor() {
		return mTransferidorC;
	}
	
	public Alinhamento getAlinhamento() {
		return mAlinhamentoC;
	}
	
	public void setFonte(int eTamanho,Color eCor) {
		HapploC = new Happlo(eTamanho, eCor);
		mColunas = CalcularColunas(mLargura, HapploC);
		mLinhas = CalcularLinhas(mAltura, HapploC);
	}
	
	public void MudarTamanho(int eTamanho) {
		setFonte(eTamanho, HapploC.getCor());
	}
	
	public void MudarCor(Color eCor) {
		setFonte(HapploC.getTamanho(), eCor);
	}
	...
	public void setLinhas(int eLinhas) {
		mLinhas=eLinhas;
	}
	...
	public void Draw(Graphics grafico) {
		...
		for (int y = 0; y < getLinhas(); y++) {
			...
			SY += HapploC.getQuadrante() + HapploC.getEspacador();
		}
	}
